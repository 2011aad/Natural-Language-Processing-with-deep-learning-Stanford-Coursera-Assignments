...
def make_windowed_data(data, start, end, window_size = 1):
	...
	for index, word in enumerate(sentence):
		...
		if i < 0:
		    ...
		    tmp_window += sentence[i]

	    windowed_data.append((tmp_window, labels[index]))		
    ### END YOUR CODE
...
class WindowModel(NERModel):
	...
	embedding_mat = tf.nn.embedding_lookup(params, self.input_placeholder)
	embeddings = tf.reshape(embedding_mat, (-1, self.config.n_window_features*self.config.embed_size))             
        ### END YOUR CODE
        return embeddings
	...
	W = tf.Variable(initializer((self.config.n_window_features*self.config.embed_size, self.config.hidden_size)), dtype = tf.float32)
	U = tf.Variable(initializer((self.config.hidden_size, self.config.n_classes)), dtype = tf.float32)
