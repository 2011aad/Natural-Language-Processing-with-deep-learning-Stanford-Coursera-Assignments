...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_x = tf.get_variable('W_x', (self.input_size, self.state_size), tf.float32, tf.contrib.layers.xavier_initializer())
	    
	    W_h = tf.get_variable('W_h', (self.state_size, self.state_size), tf.float32, tf.contrib.layers.xavier_initializer())

	    b = tf.get_variable('b', (self.state_size, ), tf.float32)
